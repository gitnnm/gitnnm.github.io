/**
 * Jackett Selector Plugin (fixed for Lampa)
 * -----------------------------------------------------------------
 * Убирает проверку Lampa.Manifest.origin === 'bylampa'
 * Исправлено преждевременное обращение к Lampa (причина script error)
 */

(function checkLampaReady() {
  // Ждём, пока Lampa загрузится
  if (typeof Lampa === 'undefined') return setTimeout(checkLampaReady, 500);
  ...
  (function() {
    ...
    function checkInstance(host, title, apiKey) {
      return new Promise(function(resolve) {
        ...
        xhr.onload = function() {
          if (xhr.status === 200) {
            resolve({ title: '✔️ ' + title, url: host, url_two: host });
          } else {
            resolve({ title: '✖️ ' + title, url: host, url_two: host });
          }
        };
    ...
    // Реакции на события настроек и storage
    ...
    // Показываем селектор при первом запуске
    showSelector();

    // Слушаем изменения выбора парсера
